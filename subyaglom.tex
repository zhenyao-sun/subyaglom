...
\]
	...
	If $\nu$ is a distribution on % $\mathbb N$ such that for any $z_0\in \mathbb Z_+$ and subset $A$ of $\mathbb N$, 
...
\end{align}
% here $\xrightarrow{w}$ stands for weak convergence. {\bf Support }
...
\end{thm}
	...
	We say a probability measure $\mathbf Q$ on $\mathcal M^o_f(E)$ is a QLD of $X$, if there exists a probability measure $\mathbf P$ on %$\mathcal M_f(E)$ such that
	$\mathcal M_f^o(E)$ such that
	\[
	(\mathbf P\mathbb P)\left(X_t \in B \middle| \|X_t\|>0\right) \xrightarrow[t\to \infty]{} \mathbf Q(B), 
	% \quad B\in \mathcal B(\mathcal M_f(E)).
	...
	$X$, 
	% for which the null measure is an absorbing state.
	...
	% Now for any $\mathcal M_f(E)$-valued Markov process with the null measure as an absorbing state, 
...
\begin{prop} \label{prop:EQ}
	...
	(2) for any $r \in (\lambda , 0)$, %there exists a probability measure $\mathbf Q_r$  on $\mathcal M_f(E)$
...
\begin{minipage}{0.9\textwidth}
	...
	%$\lim_{t\to \infty} \sup_{s\geq  0} \sup_{f\in \mathcal B(E,[0,\infty])} |C^{\eqref{eq:nVR}}_{t,s,f}| = 0$.
...
\begin{lem} \label{prop:Gtb:H1:H2:H3:H4}
	...
	%such that $G_r(\infty 1_E) = \infty$ and that
...
\end{minipage}
\end{equation}
	%ZS: I changed $1_E$ into $\mathbf 1_E$ without further noticing 
...
\begin{proof}[Proof of Proposition \ref{prop:EQ} (1)]	
	...
	% Now note that for $t> T^{\eqref{eq:EQ.1}}$,
...
thus, such a random variable exists.
	...
	% be an $\mathcal M_f(E)$-valued i.i.d. sequence 
...
\begin{thebibliography}{99}
	...
	% ZS: Following is deleted because it is not cited
	...
	% ZS: Deleted since it is not cited
	...
	% SZ: Deleted since not being cited
