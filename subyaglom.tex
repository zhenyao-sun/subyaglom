...
\label{sec:MR}
% **** Notations
Denote by $\mathcal B(E,B)$ the collection of all measurable maps from a Borel space $E$ to a Borel space $B$.
Denote by $\mathcal B_b(E,B)$ the collection of all bounded measurable maps from a Borel space $E$ to a metric space $B$.
Denote by $\mathcal M(E)$ the space of all Borel measures on a Borel space $E$. 
Denote by $\mathcal M_f(E)$ the space of all finite Borel measures on a Borel space $E$. 
...
\item
\label{def:SP:1}
  the \emph{spatial motion} $Y=\{(Y_t)_{t\geq 0};(\Pi_x)_{x\in E}\}$ is an $E$-valued Hunt process with life time $\tau$;
\item
  \label{def:SP:2}
  ...
  \]
  where $\beta, \sigma\in \mathcal B_b(E,\mathbb R)$ and $\pi$ is a kernel from $E$ to $(0,\infty)$ with
  \[
    \sup_{x\in E}\int_0^\infty (r\wedge r^2)n(x,dr)
    <\infty.
  \]
...
\begin{thm}
  ...
  We say a probability ${\mathbf P}$ on $\mathcal M(E)$ is the \emph{Yaglom distribution} of the superprocess $X$ if for each $\mu\in \mathcal M_f(E)\setminus\{0\}$ we have
  \begin{align}
    \mathbb P_\mu(X_t \in \cdot | \|X_t\|>0) 
    \xrightarrow[t\to \infty]{w} {\mathbf P}(\cdot).
  \end{align}
  {\bf Under some conditions,} it holds that the Yaglom distribution $\mathbf P$ of $X$ exists.
\end{thm}
% **** Proof of Theorem (Y)aglom-a
% ***** 
\begin{proof}[Proof of Theorem \ref{thm:Y}]

% ***** Lemma (Y)aglom: definition of (Ga)mma
\begin{lem}
  \label{lem:Y:Ga}
  {\bf Under some conditions,} there exists a family of $[0,\infty)$-valued functionals $(\Gamma_t)_{t\geq 0}$ on $\mathcal B_b(E, [0,\infty))$ such that
  \begin{align}
    e^{-\Gamma_t f}:= \mathbf P_{\nu}\big[e^{- X_t(f)}\big| \|X_t\| > 0\big], 
    \quad f\in \mathcal B_b(E,[0,\infty)), t \geq 0.
  \end{align}
\end{lem}
...
\begin{lem}
  \label{lem:Y:Gt}
  {\bf Under some conditions}, for each increasing time sequence $\mathbf t:= (t_n)_{n\in \mathbb N}$ with $ t_n \xrightarrow[n\to \infty]{} \infty$, we have that
  \begin{align}
    1 - e^{- G^{\mathbf t}V_sf} 
    = e^{s\lambda} \big(1- e^{- G^{\mathbf t} f}\big),
    \quad s \geq 0, f \in \mathcal B_b(E, [0,\infty))
  \end{align}
  where $G^{\mathbf t}$ is a $[0,\infty)$-valued functional on $\mathcal B_b(E,[0,\infty))$ given by
  \begin{align}
    G^{\mathbf t}f 
    := \limsup_{n\to \infty} \Gamma_{(t_n)}f,
    \quad f\in \mathcal B_b(E, [0,\infty)).
  \end{align}
\end{lem}

% ***** Lemma (Y)aglom: are G^t are (G*)
\begin{lem}
  \label{lem:Y:G*}
  Suppose that $G^*$ is a $[0,\infty)$-valued functional on $\mathcal B_b(E,[0,\infty))$ satisfies that
  \begin{align}
    1 - e^{- G^* V_sf} = e^{s\lambda} \big(1- e^{- G^* f}\big),
    \quad s \geq 0, f \in \mathcal B_b(E, [0,\infty)).
  \end{align}
  {\bf Under some conditions,} for each increasing time sequence $\mathbf t:= (t_n)_{n\in \mathbb N}$ with $ t_n \xrightarrow[n\to \infty]{} \infty$, we have $G^* = G^{\mathbf t}$.
\end{lem}

% ***** Lemma (Y)aglom: the unique solution (G)
\begin{lem}
  \label{lem:Y:G}
  {\bf Under some conditions,} there exists a unique $[0,\infty)$-valued functional $G$ on $\mathcal B_b(E, [0,\infty))$ satisfying that
  \begin{align}
    1 - e^{- G V_sf} = e^{s\lambda} \big(1- e^{- G f}\big),
    \quad s \geq 0, f \in \mathcal B_b(E, [0,\infty)).
  \end{align}
  Moreover, there exists a probability $\mathbf P$ on $\mathcal M_f(E)$ such that
  \begin{align}
    e^{-Gf}=\mathbf P[e^{-X(f)}],\quad f\in \mathcal B_b(E,[0,\infty)).
  \end{align}
\end{lem}

% ***** 
Denote by $G$ the functional given by Lemma \ref{lem:Y:G}. 
Then from Lemma \ref{lem:Y:G*} we have that for each increasing time sequence $\mathbf t=(t_n)_{n\in \mathbb N}$ with $t_n \xrightarrow[n\to\infty]{} \infty$, we have
\begin{align}
\limsup_{n\to \infty}\Gamma_{(t_n)} f = Gf,\quad f\in \mathcal B_b(E,[0,\infty)).
\end{align}
Notice that the limit is independent of the choice of the sequence $\mathbf t$, so we must have
\begin{align}
  \lim_{t \to \infty}\Gamma_{t} f = Gf,\quad f\in \mathcal B_b(E,[0,\infty)).
\end{align}
Using Lemma \ref{lem:Y:G} again, we get the desired result.
\end{proof}
% **** Proof of Lem:Y:Ga
% ***** 
\begin{proof}[Proof of Lemma \ref{lem:Y:Ga}]
% ***** Lemma (Mean)  
  \begin{lem}
    \label{lem:M}
    ...
    It holds that
    \begin{align} 
      \mathbb P_\mu [\langle f,X_t\rangle] = \mu(P^\beta_t f), 
...
% ***** Lemma (E)igenvalue
  \begin{lem}
    ...
    {\bf Suppose that it satisfies some conditions.}
    ...
    \begin{align}
      ...
      \\ & \nu(\phi) 
           = 1.   
    \end{align}
    We refer to $\lambda$ as the \emph{principle eigenvalue}, $\phi$ as the \emph{principle eigenfunction}, and $\nu$ as the \emph{principle eigenmeasure} of the mean semigroup of $X$, respectively. 
  \end{lem}
% ***** 
  For each $t\geq 0$, since $P_\nu[X_t(\phi)] = \nu(P_t^\beta \phi) = e^{\lambda t} > 0$, we have that $\mathbf P_\nu(\|X_t\| > 0) > 0$.
  Therefore, the conditional expectation $\mathbf P_\nu (\cdot \big|\|X_t\|>0)$ is well-defined for each $t\geq 0$.
  We only need to proof that for each $t\geq 0$ and $f\in \mathcal B_b(E,[0,\infty))$, the following hold
  \begin{align}
    \mathbf P_\nu\big[e^{-X_t(f)}\big|\|X_t\|>0\big] > 0.
  \end{align}
  In fact, if we have a time $t_0 \geq 0$ and a function $f_0 \in \mathcal B_b(E,[0,\infty))$ such that 
  \begin{align}
    \mathbf P_\nu \big[e^{- X_{t_0}(f_0)} \big|  \|X_{t_0}\| > 0\big] =0,
  \end{align}
  then we must have that $\{\|X_{t_0}\| > 0\} \subset \{X_{t_0}(f_0) = \infty\}$ in the sense of $\mathbf P_\nu$-almost surely.
  From the fact that $\mathbf P_\nu\big( X_{t_0} > 0 \big) > 0$, we have that $\mathbf P_\nu ( X_{t_0}(f_0) = \infty) > 0$.
  This, however, contradicts the fact that $\mathbf P_\nu [X_{t_0}(f_0)] = \nu(\mathbf P_{t_0}^\beta f_0) \leq \|\nu\| \cdot e^{\|\beta\|t_0} \cdot \|f_0\|_\infty < \infty$.
\end{proof}
% **** Proof of Lem:Y:Gt
% **** Proof of Lem:Y:G*
% **** Proof of Lem:Y:G
...
For each $x\in E$, let the probability $(\widetilde \Pi_{x})_{x\in E}$ be Doob's $h$-transform of $\Pi_x$ such that
\begin{align}
...
For each $\mu \in \mathcal M_f(E)$, define
\[
	\Pi_{\mu}(\cdot)
...
\]
For each function $f \in \mathcal B_b(E,[0,\infty))$ and measure $\mu \in \mathcal M_f(E)$, define measure $( f \cdot\mu)$ such that
\[
  (f \cdot \mu)(dx)
  := f(x)\mu(dx),
  \quad x\in E.
...
\begin{lem}
	For each $\mu\in \mathcal M_f(E)$, we have
 \[
...
\begin{align} 
\begin{multlined} \mathbb D:=\{ w= (w_t)_{t\geq 0}: w \text{ is an $\mathcal M_f(E)$-valued c\`{a}dl\`{a}g function on $[0,\infty)$ }
...
\item
  for each $\mu \in \mathcal M_f(E)$, if $\mathcal N$ is a Poisson random measure on $\mathbb D$ with intensity
  \[
...
In the remainder of this paper, we will always use $w = (w_t)_{t\geq 0}$ to denote a generic element in $\mathbb D$.
For each $f\in \mathcal B_b(\mathbb D, [0,\infty))$, from Campbell's formula it can be verified that for each $\mu\in \mathcal M_f(E)$ and $t>0$, we have
\begin{align}\label{eq: kuznetsov Laplace}
...
Fix an arbitrary $\mu\in \mathcal M_f(E)$.
...
% **** (Lemma about the mean semigroup)
\begin{comment}
\begin{lem}
...
For each $\epsilon > 0$, we have
  \begin{align}
    P^\beta_t f(x) = e^{\lambda t} \phi(x) \nu(f)\left( 1 + o(\limsup_{t\to \infty}\sup_{ x\in E, f\in L_1^+(\nu)})\right).  
  \end{align}
\end{lem}
\begin{proof}
...
\begin{lem}
  \label{lem:K}
  Let $X$ be the superprocess given by Definition \ref{def:SP}.
  Suppose that it satisfies the condition of Lemma \ref{lem:S} {\bf and some other conditions}.
  ...
  \begin{align}
    ...
    \label{eq:CK}
    P_t^\beta f(x) 
    & = e^{\lambda t} \phi(x) \nu(f) \Big( 1+ o\big(\limsup_{t\to \infty} \sup_{x\in E, f\in L^1(\nu)}\big)\Big), \\
    P_t^\beta f(x) 
    & = \phi(x) \nu(f) O\big(\sup_{x\in E, f\in L^1(\nu)}\big), 
      \quad t >0.
  \end{align}
...
\begin{proof}[Proof of Lemma \ref{lem:K}.(\ref{lem:K:1})]
In this proof we write $f$ for a fixed function chosen abitrary in $\mathcal B(E,[0,\infty])$.
Define a function $\psi_0$ on $E\times [0,\infty)$ by
\begin{align}
  \psi_0(x,z) 
  := \psi(x,z)+ \beta(x) z,
  \quad x\in E, z \geq 0,
\end{align}
and an operator $\Psi_0$ on $B(E,[0,\infty))$ by
\begin{align}
\Psi_0 g(x):=\psi_0(x, g(x)), \quad g\in \mathcal B(E,[0,\infty)).
\end{align}
...
\begin{align}
  ...
  \quad s\geq 0, g\in \mathcal B_b(E, [0,\infty)).
\end{align}
\end{lem}

\begin{lem}
\label{lem:VfS}
For each $f\in \mathcal B(E, [0,\infty])$ and $t>0$, we have $V_tf \in \mathcal B_b(E, [0,\infty))$.
\end{lem}
In particular, taking $g=V_tf$ we have,
\begin{equation}
  \label{eq:EVf}
...
\begin{lem}
  We have that
  \begin{align}
    \nu(V_t g) = O(\limsup_{t\to \infty} \sup_{g\in \mathcal B(E,[0,\infty])})
  \end{align}
\end{lem}

For the rest of the proof, denote by $t_0>0$ a large enough number such that
\begin{align}
\sup_{t\geq t_0, g \in \mathcal B(E,[0,\infty])} \nu(V_tg) < \infty.
\end{align} 

Therefore from \eqref{eq:CK} we have 
\begin{align}
  &P_s^\beta V_t f(x) = e^{\lambda s} \phi(x) \nu(V_tf) \left (1+o(\limsup_{s\to \infty}\sup_{t\geq t_0,x\in E})\right), \label{eq:asmptotic_for_PsVtf}
\end{align}
...
\leq P_t^\beta V_tf(x)
\\&= e^{\lambda t} \phi(x) \nu(V_tf) \left( 1+ o(\limsup_{t\to \infty} \sup_{x\in E})\right)
 \\& \leq e^{\lambda t} \phi(x) O(\limsup_{t\to \infty} \sup_{x\in E})
\end{align}
This implies the desired result.
\end{proof}

\begin{proof}[Proof of Lemma \ref{lem:K}.\eqref{lem:K:2}]
...
Therefore, for each $x\in E$, $\psi_0(x,z)$ is a convex function in $z$, and thus from $\psi_0(x,0) = 0$, it holds that
\begin{align}
\psi_0(x,z) \leq z \psi_0'(x,z),
\quad z \geq 0.
\label{eq:6}
\end{align}

Define an operator $\Psi'_0$ on $\mathcal B(E,[0,\infty))$ such that
\begin{align}
\Psi'_0 f(x) := \psi_0(x,f(x)), \quad x\in E.
\end{align}

From the monotonic convergence we have
\begin{align}
...
\begin{lem}
\label{lem:TTE}
There exists a $t_0>0$ such that for each $f\in \mathcal B_b(E,[0,\infty])$ and each $t > s > t_0$, we have
\begin{align}
e^{-\lambda t} \nu(V_tf) 
= e^{\lambda s} \nu(V_sf) \exp\Big\{- \int_s^t \frac{\nu(\Psi_0 V_u f)}{\nu(V_uf)} du\Big\}.
\end{align}
\end{lem}
...
\begin{align}
  ...
  \to \infty}\sup_{x\in E}) \phi(x) \nu(V_tf),\quad\text{by~\eqref{eq:Pv_and_vnu}} 
\end{align}
...
% **** Proof of lemma TTE
\begin{proof}[Proof of Lemma \ref{lem:TTE}]
...
\begin{cor}\label{cor:general_rate}
	For each $\mu\in\mathcal M_f(E)\setminus\{0\}$, $f\in\mathcal B_b(E,[0,\infty])$ and $s>0$, it holds that
...
\begin{lem}\label{eq:ratio_limits_2}
	For any $f\in\mathcal B_b(E,[0,\infty))$, there is a nonnegative function $G(f)$ such that $G(v_t)=t$ and such that for any $\mu\in\mathcal M_f(E)\setminus\{0\}$,
...
\quad t\geq 0.
  \end{align}
As a result, for any $\mu\in\mathcal M_f(E)\setminus\{0\}$,
...
\begin{proof}[{\bf Proof of Theorem \ref{thm:distribution_of_zeta}}]
	For any finite measure $\mu\in \mathcal M_f(E)$,
  \[
  ...
  \]
	...
	Meanwhile, thanks to \eqref{ext con}, for any $\mu,\widetilde\mu\in \mathcal M_f(E)\setminus\{0\}$ and $s\in\mathbb R$,
  \[
...
\begin{proof}[{\bf Proof of Theorem \ref{thm:qsd_thm}}]
  For any nonzero $\mu\in \mathcal M_f(E)$, and any $f\in\mathcal B_b(E,[0,\infty))$,
  \begin{eqnarray*}
  ...
  \]
  When $f=0$, $G(f)=\infty$. Thus $1-e^{\lambda G(f)}$ is a Laplace functional of probability on $\mathcal M_f(E)$. We denote by $\mathbf P^{\lambda}$ the corresponding probability, then
  \[
  ...
  $$
  \mathcal S:=\{\mu\in\mathcal M_f(E); \mu(dx)=a\widehat\phi(x)m(dx), a\geq 0\}.
  $$
  ...
  \[
    \mathbf P\mathbb P(\zeta>t)=1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\omega\rangle }\mathbf P(d\omega)=e^{\gamma t}.
  \]
  ...
  \begin{eqnarray*}
    \mathbf P\mathbb P\left(e^{-\langle f, X_t\rangle }\big|\zeta>t\right)&=&1-e^{-\gamma t}\mathbf P\mathbb P\left(1-e^{-\langle f, X_t\rangle }\right)\\
                                                                          &=&1-e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle V_tf,\omega\rangle }\mathbf P(d\omega)\right)\\
                                                                          &=&\int_{\mathcal M_f(E)}e^{-\langle f,\omega\rangle }\mathbf P(d\omega).
  \end{eqnarray*}
  Recall that $\lim_{t\rightarrow\infty}\dfrac{\langle v(t+s,\cdot),\omega\rangle }{\langle v_t,\nu\rangle }=\langle \phi,\omega\rangle e^{\lambda s} $ and that $\lim_{t\rightarrow\infty}\dfrac{\langle V_{t+s}f,\omega\rangle }{\langle V_tf,\nu\rangle }=\langle \phi,\omega\rangle e^{\lambda s}$ uniformly for all $\omega\in \mathcal M_f(E)\setminus\{0\}$.  For any $\varepsilon>0$, there is $T>0$, such that when $t>T$,
  \[
    (1-\varepsilon)\langle v_t,\nu\rangle \langle \phi,\omega\rangle\leq \langle v_t,\omega\rangle\leq (1+\varepsilon)\langle v_t,\nu\rangle \langle \phi,\omega\rangle,\qquad \omega\in\mathcal M_f(E).
  \]
  Therefore, on one hand,
  \begin{eqnarray*}
    1&=&e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\omega\rangle }\mathbf P(d\omega)\right)\\
     &\leq &e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\nu\rangle\langle\phi,\omega\rangle }\mathbf P(d\omega)\right)+e^{-\gamma t}\int_{\mathcal M_f(E)}\left(1-e^{-\varepsilon\langle v_t,\nu\rangle\langle\phi,\omega\rangle  }\right)\mathbf P(d\omega)\\
     &=&I_t+II_t.
  \end{eqnarray*}
  For the given $\varepsilon>0$, choose $s>0$, such that $e^{\lambda s}/2>\varepsilon$.  And there is $T_1>0$, such that $t>T_1$,
  \[
    \varepsilon\langle v_t,\nu\rangle\langle\phi,\omega\rangle\leq \langle v(t+s,\cdot),\omega\rangle \quad\mbox{for all}\, \omega\in\mathcal M_f(E).
  \]
  So
  \begin{eqnarray*}
    0\leq II_t&\leq& e^{-\gamma t}\int_{\mathcal M_f(E)}\left(1-e^{-\varepsilon\langle v_t,\nu\rangle\langle\phi,\omega\rangle  }\right)\mathbf P(d\omega)\\
              &\leq& e^{-\gamma t}\int_{\mathcal M_f(E)}\left(1-e^{-\langle v(t+s,\cdot),\omega\rangle  }\right)\mathbf P(d\omega)\\
              &=&e^{\gamma s}.
  \end{eqnarray*}
  ...
  \begin{eqnarray*}
    1&=&e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\omega\rangle }\mathbf P(d\omega)\right)\\
     &\geq &e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\nu\rangle\langle\phi,\omega\rangle }\mathbf P(d\omega)\right)-e^{-\gamma t}\int_{\mathcal M_f(E)}\left(1-e^{-\varepsilon\langle v_t,\nu\rangle\langle\phi,\omega\rangle  }\right)\mathbf P(d\omega)\\
     &=&I_t-II_t.
  \end{eqnarray*}
  It follows that $\limsup_{t\to\infty}I_t\leq 1$. In conclusion,
  \begin{equation}\label{eq:limit1}
    \lim_{t\rightarrow\infty}e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle v_t,\nu\rangle\langle\phi,\omega\rangle }\mathbf P(d\omega)\right)=1.
  \end{equation}
  Repeating the arguments for \eqref{eq:limit1}, we can deduce the following limit
  \begin{equation}\label{eq:limit2}
    \lim_{t\rightarrow\infty}e^{-\gamma t}\left(1-\int_{\mathcal M_f(E)}e^{-\langle V_tf,\nu\rangle \langle \phi,\omega\rangle }\mathbf P(d\omega)\right)=1-\int_{\mathcal M_f(E)}e^{-\langle f,\omega\rangle }\mathbf P(d\omega).
  \end{equation}
  Since $\langle V_tf,\nu\rangle =\langle v(t+G(f),\cdot),\nu\rangle $, combining \eqref{eq:limit1} and \eqref{eq:limit2}, we get
  \begin{equation}\label{eq:lap_qsd}
    \int_{\mathcal M_f(E)}e^{-\langle f,\omega\rangle }\mathbf P(d\omega)=1-e^{\gamma G(f)}.
  \end{equation}
  ...
  \[
    \int_{\mathcal M_f(E)}\langle f,\omega\rangle e^{-r\langle f,\omega\rangle }\mathbf P(d\omega)
    =-\gamma \langle f,\nu\rangle  e^{\gamma B(r\langle f,\nu\rangle)}B'(r\langle f,\nu\rangle).
  \]
  Therefore,
  \[
    \int_{\mathcal M_f(E)}\langle f,\omega\rangle \mathbf P(d\omega)=-\gamma \langle f,\nu\rangle  \lim_{r\rightarrow 0+}e^{\gamma B(r)}B'(r)
  \]
  When $\gamma<\lambda$,
  \[
  ...
  \[
    \int_{\mathcal M_f(E)}\langle f,\omega\rangle \mathbf P^{\gamma}(d\omega)=0.
  \]
  So there is no QSD associated to the rate of mass decay $-\gamma$, for $\gamma<\lambda$.
\end{proof}

%%% -------------------------------------------------------------------------------------------------------------------------------------------------
{\color{blue}According to the definition for QSD in \cite{ChampagnatVillemonais2018Convergence}, there should be some probability measure $\mathbf Q^\gamma$ on $\mathcal M_f(E)$, $\gamma\in[\lambda, 0)$ such that
  \[
...
\begin{proof}
  [Proof of Theorem \ref{thm:Q_process}]
...
\begin{proof}[Proof of Theorem \ref{thm:structure_of_Qprocess}]
  ...
  Now let us consider the cases for general initial value $\mu\in\mathcal M_f(E)$.  For $f\in\mathcal B_b(E,[0,\infty))$, define function
  ...
  \begin{equation*}
    ...
    =\widehat{\mathbb Q}_{\nu}\left(e^{-\langle f, X_{\infty}\rangle }\right).
  \end{equation*}
  Therefore for any $\mu\in\mathcal M_f(E)\setminus\{0\}$, and $f\in\mathcal B_b(E,[0,\infty))$,
  \begin{eqnarray*}
  ...
  Thus in this case, the limit measure $X_\infty\in \mathcal M_f(E)$.  Denote the distribution of $X_\infty$ by $\mathbf P$.  Then $\mathbf P$
  ...
  $X_\infty\in\mathcal M_f(E)$, as $t\to\infty$. Therefore
  \[
  ...
  \]
  In this case, the $Q$ process does not have invariant probability. And $\langle \phi, X_t\rangle $ converges to $\infty$ as $t\to\infty$ in probability with respect to $\widetilde{\mathbb P}_\mu$ for any $\mu\in \mathcal M_f(E)\setminus\{0\}$.
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ----------------------------------------------------------------------------------------------------------------
