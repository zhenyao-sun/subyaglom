...
[Ren:] Prefer "invariant probability"
  \end{itemize}
\end{iss}
\begin{iss}[open]~
  \begin{itemize}
...
[Ren:]  It is safe to say stationary because we have not prove uniqueness.
  \end{itemize}
\end{iss}
...
[Ren:] Yes, we should introduce more on it .
  \end{itemize}
\end{iss}
...
\label{iss:notation_for_pt}
  ...
  \item[ZS:]
    ...
    [Ren:] These notations are OK. We may just say that $\widehat p_t$ is some time written as $\widehat p(t,\cdot,\cdot)$. Similarly for others.
  \end{itemize}
\end{iss}
...
[Ren:] It should only holds with $\epsilon = -\lambda$, for for any $\epsilon>0.$
  \end{itemize}
\end{iss}
