...
\]
	...
	% If $\nu$ is a distribution on $\mathbb N$ such that for any integer $z_0>0$ and 
...
\[ 
	%\int_{[0,\infty)} \nu(x)\mathbb P_x (Z_t \in A | \zeta > t) = \nu (A) 
...
\] 
	...
	%Asmussen and Hering \cite{AH} studied limit behaviors  subcritical branching Markov processes in which the lifetimes and offspring distribution are independent of the spatial motion. 
	%{(\bf ZS: I think they allow dependence of the spatial motions.) (\bf RL: I agree with Zhenyao)}
	%They proved that the Yaglom limit for these subcritical  branching Markov processes exists under some conditions on the mean semigroup, see \cite[Chapter 5]{AH} and reference there in.
...
\begin{prop} \label{prop::GD:H1:H2:H3:H4::Y}
	...
	%we have $G g_n \downarrow g$ pointwisely. 
...
\end{equation}
	...
	% {\bf (RL: I can not assure $(1 - e^{- \gamma_{t_n'} f})$ have positive lower bound. So I can not see how we get this kind of convergence. Or we may need to point out $\Gamma_{(t_n')}f$ is bounded.)}
...
\begin{align}
	...
	% \\&\overset{\eqref{eq:vp.5},\eqref{lem:Gfnv!}}= \frac{1 - e^{- \nu( V_{(t_n')}f)}}{1- e^{- \nu(v_{(t_n')})}}  (1+C^{\eqref{eq:vp.5}}_n)
...
\begin{equation} \label{eq:vp.61}
	1 - e^{- G^{\mathbf t}f}
	% \overset{\text{ Proposition \ref{prop:Vf2}}}=  \frac{V_{(t_n')}f(x)}{v_{(t_n')}(x)} \frac{1+C^{\ref{prop:Vf2}}_{t_n',x,\infty \mathbf 1_E}}{1+C^{\ref{prop:Vf2}}_{t_n',x,f}} (1+C^{\eqref{eq:vp.6}}_n).
...
\begin{align} 
	...
	% \\ \label{eq:GD.n2} &\overset{\eqref{eq:GD.n1}}\geq \frac{1-\epsilon}{1+\epsilon} \frac{\nu(V_tf)}{\nu(V_th)} V_th(x).
...
\begin{align}
	...
	% \overset{\eqref{eq:GD.n2}}\geq e^{-\lambda t}\left(1- e^{- G\left(\frac{1-\epsilon}{1+\epsilon} \frac{\nu(V_tf)}{\nu(V_th)} V_th\right) }\right)
	...
	% \\ &\overset{\eqref{eq:GD.n3}}\geq e^{-\lambda t}\frac{1-\epsilon}{1+\epsilon} \frac{\nu(V_tf)}{\nu(V_th)} \left(1- e^{- G\left(V_th\right) }\right)
	\\ &\overset{\eqref{eq:GD.n3}}\geq e^{-\lambda t} \left(\frac{1-\epsilon}{1+\epsilon} \frac{\nu(V_tf)}{\nu(V_th)} \wedge 1 \right) \left(1- e^{- GV_th }\right)
	% \overset{\text{Proposition \ref{prop:G}}}= \frac{1-\epsilon}{1+\epsilon} \frac{\nu(V_tf)}{\nu(V_th)} (1 - e^{- Gh}).
