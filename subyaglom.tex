...
Write $\langle f,\mu\rangle$ or$\mu(f)$ for the integral $\int_E f(x)\mu(dx)$ for the function $f$ and measure $\mu$ whenever it make sense.
...
\end{asp}
 ...
 The above assumption says that, with a positive probability, the process will extinct in finite time, provided the initial configuration is $\nu$ or $\delta_x$ with $x\in E$.
...
< \left\langle v_t,\nu \right\rangle.  
\end{align}
Moreover, Lemma \ref{lem:asmptotic_of_vtf}.\eqref{sublem:Vtf_vanish} says that
  \[
...
\begin{proof}
  ...
  Then from \eqref{eq:equation_for_Vtf} we have....
  \begin{iss}[TODO]~
    \begin{itemize}
    \item[ZS:]
...
\begin{align}
  ...
  \\& = e^{\lambda B(\left\langle f, \nu \right\rangle)},\quad\text{by~Corollaray \ref{lem:extinct}.\eqref{sublem:extinct_2_1}}
\end{align}
\end{proof}

\begin{proof}[Proof. Version B.]
	Thanks to Corollaray \ref{cor:general_rate} we only need to prove
  \begin{equation}\label{eq:spec_ratio}
    \lim_{t\rightarrow\infty}\frac{\langle V_tf,\nu\rangle }{\langle v_t,\nu\rangle }=e^{\lambda G(f)}.
  \end{equation}
