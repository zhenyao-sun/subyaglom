...
V_tf(x)
=\phi(x) \nu (V_tf) 
%\chi(\lim_{t\to \infty} \sup_{x\in E}).
...
%the equation in Fact \ref{Fact:P!} with respect to $\nu$, 
...
\begin{align}
  &\Psi_0' V_tf(x)
%  \leq 2\alpha (x)^2 V_t f(x) + V_t f(x) \int_0^1 r^2 \pi(x,dr) + 
  \leq 2\sigma (x)^2 V_t f(x) + V_t f(x) \int_0^1 r^2 \pi(x,dr) + 
  %YX 2\int_1^\infty r \pi(x,dr)
...
%YX There exists $t_0>0$ such that $V_t f(x ) = 0$ for each $x\in E$, $t > t_0$ and $f \in \mathcal B(E,[0,\infty])$ with $\nu(f) = 0$.
%If $f \in \mathcal B(E,[0,\infty])$ with $\nu(f) = 0$, then there exists $t_0>0$ such that $V_t f(x ) = 0$ for each $x\in E$, $t > t_0$.
...
%as long as $t>0$ is large enough, we have $\nu(\Psi_0 V_t f)< \infty$.
...
as desired.
\end{proof}
%YX Now, in order to proof Proposition \ref{Proposition:JVf:H1:H2:H3:H4}, we only have to proof Claim \ref{Claim:PPV:H1:H2:H3:H4}.
...
as desired.
\end{proof}
%YX Now we only have to proof Claim \ref{Claim:PuPVt:H1:H2:H3:H4}.
...
%YX From Lemma \ref{Lemma:VfO:H4:H3} and \ref{Proposition:nVR:H1:H2:H3:H4} we get
...
%proof Proposition \ref{Proposition:Gtb:H1:H2:H3:H4} and \ref{Proposition:G:H1:H2:H3:H4}.
...
\begin{align}
 Q_t \Big( \frac{V_tf}{v_t}  \Big)
   ...
   = 1- e^{- G^* f}.
 \end{align}
%Therefore, taking $n \to \infty$ in the previous inequality, 
...
\quad x\in E.
\end{align}
%YX From this we can verify that 
% From this we have that 
%for each $s\geq 0$ and $\epsilon >0$ there exists $t>0$ large enough such that for each $u \in [0,\infty)$,
...
%YX we get from Leamma \ref{Lemma:QM!} that, 
...
%YX and the monotone convergence theorem: $e^{-\nu(v_t)} = \mathbb P_\nu (\|X_t\| = 0) \xrightarrow[t\to \infty]{} 0$.
